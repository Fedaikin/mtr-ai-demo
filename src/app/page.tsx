...
function AudienceMetric({ label, value, hint, tone = "default" }: { label: string; value: string | number; hint: string; tone?: "default" | "success" | "warning" | "danger" }) {
  ...
  return <div className={`rounded-lg border p-4 shadow-sm ${styles}`}><p className="text-xs font-medium text-slate-500">{label}<InfoHint title={label} /></p><p className="mt-1 text-2xl font-semibold tabular-nums text-slate-950">{value}</p><p className="mt-1 text-[11px] text-slate-500">{hint}</p></div>;
}

function DashboardCard({ eyebrow, title, children }: { eyebrow: string; title: string; children: ReactNode }) {
  return <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wide text-teal-700">{eyebrow}</p><h3 className="mt-1 text-lg font-semibold text-slate-950" aria-label={title}>{title}<InfoHint title={title} /></h3><div className="mt-3">{children}</div></section>;
}
...
function DashboardMetric({ label, value, hint, warning = false }: { label: string; value: number; hint: string; warning?: boolean }) { return <div className={`rounded-lg border p-4 shadow-sm ${warning ? "border-amber-200 bg-amber-50" : "border-slate-200 bg-white"}`}><p className="text-xs text-slate-500">{label}<InfoHint title={label} /></p><p className="mt-1 text-2xl font-semibold tabular-nums">{formatNumber(value)}</p><p className="mt-1 text-[11px] text-slate-500">{hint}</p></div>; }
function OperationalFact({ label, value }: { label: string; value: string | number }) { return <div className="rounded-md bg-slate-50 p-2"><p className="text-slate-500">{label}<InfoHint title={label} /></p><p className="mt-1 font-semibold text-slate-900">{value}</p></div>; }
