...
function SmallFact({ label, value, danger = false }: { label: string; value: number; danger?: boolean }) { return <div className="rounded-md bg-slate-50 p-2"><p className="text-slate-500">{label}<InfoHint title={label} /></p><p className={`mt-1 text-base font-semibold tabular-nums ${danger ? "text-red-700" : "text-slate-900"}`}>{value}</p></div>; }
