      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 id="agent-workspace-title" className="mt-1 text-xl font-semibold text-slate-950" aria-label="Рабочее пространство МТР-агента">Рабочее пространство МТР-агента<InfoHint title="Рабочее пространство МТР-агента" /></h2>
...
function CasesPanel({ items, unavailable, onContinue }: { items: readonly PublicAgentCase[]; unavailable: boolean; onContinue: (item: PublicAgentCase) => void }) {
  ...
  return <div className="space-y-3"><PanelTitle title="Личные кейсы" detail="Только активный проект и текущий пользователь" />{items.map((item) => <CaseCard key={item.id} item={item} onContinue={onContinue} />)}</div>;
}

function CaseCard({ item, onContinue }: { item: PublicAgentCase; onContinue: (item: PublicAgentCase) => void }) {
  const history = item.contextSnapshot.analysisHistory;
  return <article className="rounded-lg border border-slate-200 p-3"><div className="flex items-start justify-between gap-3"><h3 className="text-sm font-semibold text-slate-900">{item.title}</h3><StatusBadge>{caseStatusLabel(item.status)}</StatusBadge></div><p className="mt-2 text-xs text-slate-500">Обновлён {formatDateTime(item.updatedAt)} · источников {item.evidence.length}</p>{history ? <div className="mt-3 rounded-lg bg-slate-50 p-3"><p className="text-[11px] font-semibold uppercase tracking-wide text-teal-800">История расчёта</p><p className="mt-1 text-xs leading-5 text-slate-700">{history.summary}</p><p className="mt-2 text-[11px] text-slate-500">Данные {history.datasetVersion} · модель {history.forecastModelVersion ?? "прогноз недоступен"} · источников {history.sourceCount}</p>{history.changedConclusion !== null ? <p className={`mt-1 text-[11px] font-medium ${history.changedConclusion ? "text-amber-800" : "text-emerald-800"}`}>{history.changedConclusion ? "Вывод изменился с прошлого расчёта" : "Вывод подтверждён повторным расчётом"}</p> : null}</div> : null}{item.revokedEvidenceCount > 0 ? <p className="mt-2 text-xs font-medium text-amber-800">Скрыто источников после повторной авторизации: {item.revokedEvidenceCount}</p> : null}<button type="button" onClick={() => onContinue(item)} className="focus-ring mt-3 rounded-md border border-teal-200 px-3 py-1.5 text-xs font-semibold text-teal-800 hover:bg-teal-50">Продолжить в этом контексте</button></article>;
}
...
function PanelTitle({ title, detail }: { title: string; detail: string }) { return <header><h2 className="font-semibold text-slate-950" aria-label={title}>{title}<InfoHint title={title} /></h2><p className="mt-1 text-xs leading-5 text-slate-500">{detail}</p></header>; }
...
function PanelEmpty({ title, detail }: { title: string; detail: string }) { return <div className="py-14 text-center"><h2 className="font-semibold text-slate-900" aria-label={title}>{title}<InfoHint title={title} /></h2><p className="mx-auto mt-2 max-w-xs text-sm leading-6 text-slate-500">{detail}</p></div>; }
