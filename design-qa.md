...
# Design QA — закрытие окна МТР-агента

- Источник: `/var/folders/1n/9pxbg4795vz0wrv5sp7dm_lc0000gp/T/codex-clipboard-3796f644-e3fe-4202-ab97-906767085b9f.png`
- Проверяемый сценарий: открыть виджет «МТР-агент», увидеть кнопку закрытия в правом верхнем углу шапки, закрыть окно мышью и клавиатурой.
- Эталонное состояние: desktop, открытый диалог, заголовок «Новый вопрос».
- Обязательные поверхности: позиция и размер кнопки; отсутствие наложения на статус; читаемый hover/focus; доступное имя; фактическое закрытие окна.

## История проверки

1. Исходный скриншот просмотрен полностью: в шапке справа показан только статус, отдельной кнопки закрытия нет.
2. Реализация добавляет компактную кнопку 36×36 px с иконкой `X` после статуса; кнопка имеет `aria-label`, `title`, hover и общий project focus ring.
3. Unit regression test подтверждает вызов обработчика закрытия.
4. Визуальная и runtime-проверка Vercel Preview будет добавлена после первого push.

## Текущий результат

Ожидает Preview evidence. Final result будет выставлен после визуального сравнения и проверки закрытия в браузере.
