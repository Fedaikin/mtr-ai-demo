...
- Eval gate мастер-промпта локально закрыт: выполнены 34 legacy, 50 current-runtime analytical, 17 learning lifecycle, 20 provider-boundary, 32 security-boundary, 20 scale и 27 multi-turn cases (`200/200`). Analytical pack содержит 20 отдельных root-cause/rolling-backtest/scenario-ranking oracle и 20 held-out cases; security pack — permission/scope/injection/reauthorization; scale pack — 20 distinct portfolio cases с concurrency 10; multi-turn pack — 15 sensitivity и 12 feedback conversations. E2E ≥40 и exact-SHA Preview остаются отдельными незакрытыми release gates.
